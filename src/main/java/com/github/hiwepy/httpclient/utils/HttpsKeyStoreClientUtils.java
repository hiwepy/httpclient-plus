...
import com.github.hiwepy.httpclient.HttpClientConnectionManagerBuilder;
import com.github.hiwepy.httpclient.handler.response.AbstractResponseHandler;
...
public abstract class HttpsKeyStoreClientUtils extends HttpsClientUtils{
		...
		cachedConnectionManager = CacheBuilder.newBuilder()
			...
			.build(new CacheLoader<KeyStore,HttpClientConnectionManager>() {
				...
				public HttpClientConnectionManager load(KeyStore trustStore) throws Exception {
					//安全校验的Http连接池
					return  HttpClientConnectionManagerBuilder.getInstance(config).getSafeSSLConnectionManager(trustStore);
				}
			});
	}
     ...
     *@修改描述:
     */
    public static CloseableHttpClient getCloseableHttpClient(HttpClientBuilder clientBuilder, KeyStore trustStore){
    	try {
			return clientBuilder.build();
		} catch (Exception e) {
			//创建忽略任何安全校验的httpClient实例.
			return HttpClientSSLUtils.getInsecureSSLClient()
	...
	public static <T> T  httpsRequestWithGet(KeyStore trustStore,String baseURL, Map<String, Object> paramsMap,AbstractResponseHandler<T> handler) throws  IOException {
		return httpsRequestWithGet(trustStore,baseURL, paramsMap, config.getCharset(),handler);
	}
	...
	public static <T> T  httpsRequestWithPost(KeyStore trustStore,String baseURL,Map<String, Object> paramsMap,AbstractResponseHandler<T> handler) throws IOException {
		return httpsRequestWithPost(trustStore,baseURL, paramsMap, config.getCharset(),handler);
	}
